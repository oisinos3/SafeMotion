...
def open_folder_in_explorer(path):
    """Opens a given folder path in the system's default file explorer."""
...
# --- Reusable Bake Function ---
def bake_all_tracked_rigs(context):
    """
    Bakes animation for all armatures currently being tracked by the addon.
    This function is called by the bake operator and the pipeline operators.
    """
    tracked_armatures = list(context.scene.optitrack_tracked_armatures)
    if not tracked_armatures:
        # This function doesn't report to the user, the calling operator will.
        return False, 0
...
class OBJECT_OT_bake_tracked_rigs(bpy.types.Operator):
    ...
    def execute(self, context):
        success, baked_count = bake_all_tracked_rigs(context)
        if not success:
            self.report({'WARNING'}, "No armatures are being tracked to bake.")
            return {'CANCELLED'}

        self.report({'INFO'}, f"Baked {baked_count} tracked armatures.")
        return {'FINISHED'}
...
class OBJECT_OT_export_all_rigs_for_unreal(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        context.scene.optitrack_tracked_armatures.clear()
        self.report({'INFO'}, f"Exported {export_count} armatures to {export_path} and cleared tracked list.")
        
        open_folder_in_explorer(export_path)
...
class OBJECT_OT_export_selected_rigs_for_unreal(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        context.scene.optitrack_tracked_armatures.clear()
        self.report({'INFO'}, f"Exported {export_count} selected armatures to {export_path} and cleared tracked list.")
...
# --- OPERATOR 6: Full Pipeline (Import -> Create -> Bake -> Export) ---
class OBJECT_OT_run_full_pipeline(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        # --- FIX: Capture objects before import to identify new ones ---
        objects_before = set(context.scene.objects)
        ...
        # --- FIX: Identify newly imported objects ---
        objects_after = set(context.scene.objects)
        new_objects = list(objects_after - objects_before)
        if not new_objects:
            self.report({'WARNING'}, "FBX file was empty or importer failed.")
        ...
        # --- FIX: Set up scene collections just like the manual import operator ---
        base_name = bpy.path.display_name_from_filepath(self.filepath)
        main_coll = bpy.data.collections.new(base_name)
        context.scene.collection.children.link(main_coll)
        skel_coll = bpy.data.collections.new("Skeleton")
        ...
        # --- FIX: Set the scene frame range based on the new animation ---
        min_frame = float('inf'); max_frame = float('-inf'); found_animation = False
        for obj in new_objects:
            if obj and obj.animation_data and obj.animation_data.action:
                found_animation = True
        ...
        # --- MODIFICATION: Use the already identified new_objects for efficiency ---
        empties = [o for o in new_objects if o.type == 'EMPTY' and '_bone' in o.name.lower()]
        if not empties:
        ...
        for original_empty in empties:
            ...
            # --- MODIFICATION: The collection logic can be simplified now ---
            # The parent collection for the new "Rigidbody Skeletons" collection
            # will be the main collection we created for this import.
        ...
        # 3) Bake
        success, _ = bake_all_tracked_rigs(context)
        if not success:
            self.report({'ERROR'}, f"Bake failed. No rigs were tracked.")
            return {'CANCELLED'}

        # 4) Export
        # --- MODIFIED: Export path is now relative to the .blend file for consistency ---
        base_dir = os.path.dirname(bpy.data.filepath) if bpy.data.filepath else os.path.abspath('.')
        if base_dir == '.' and not bpy.data.filepath:
            self.report({'WARNING'}, "Please save the .blend file to set an export location.")
        ...
        context.scene.optitrack_tracked_armatures.clear()
        self.report({'INFO'}, f"Full pipeline completed. Exported {export_count} FBX(s) to {export_path} and cleared tracked list.")
...
# --- OPERATOR 7: Post-Import Pipeline (Create -> Bake -> Export) ---
class OBJECT_OT_run_post_import_pipeline(bpy.types.Operator):
    """Runs the pipeline on already imported/existing objects: creates armatures from _bone empties, bakes them, and exports FBX files for Unreal."""
    bl_idname = "object.run_post_import_optitrack_pipeline"
    bl_label = "Run Post-Import OptiTrack Pipeline" # Label will be overwritten in draw for consistency
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        # 1) Get empties from ALL in scene
        bone_empties_to_process = [obj for obj in bpy.data.objects if obj.type == 'EMPTY' and "_bone" in obj.name.lower()]

        if not bone_empties_to_process:
            self.report({'INFO'}, "No empties containing '_bone' were found in the scene.")
            return {'CANCELLED'}

        # 2) Create rigs from empties containing '_bone'
        processed_parent_collections = {}
        for original_empty in bone_empties_to_process:
            lower_name = original_empty.name.lower()
            bone_index = lower_name.find('_bone')
            prefix = original_empty.name[:bone_index]
            suffix = original_empty.name[bone_index + 5:]
        ...
        # 3) Bake
        if not context.scene.optitrack_tracked_armatures:
            self.report({'WARNING'}, "No new armatures were created to bake.")
            return {'CANCELLED'}

        success, _ = bake_all_tracked_rigs(context)
        if not success:
            self.report({'ERROR'}, f"Bake failed. No new rigs were tracked to bake.")
            return {'CANCELLED'}
        ...
        context.scene.optitrack_tracked_armatures.clear()
        self.report({'INFO'}, f"Post-import pipeline completed. Exported {export_count} FBX(s) to {export_path} and cleared tracked list.")
...
class VIEW3D_PT_optitrack_rigidbody_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col_export.prop(context.scene, "optitrack_export_folder_name", text="Export Folder")
...
class VIEW3D_PT_optitrack_automation_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        note_box = box.box()
        note_box.label(text="Pipelines export next to your saved .blend file.", icon='INFO')
        
        box.separator()

        box.operator(OBJECT_OT_run_full_pipeline.bl_idname, text="Run Full Pipeline (from file)", icon='PLAY')
        box.operator(OBJECT_OT_run_post_import_pipeline.bl_idname, text="Run on Imported Scene", icon='PLAY')
...
classes = (
    ...
    OBJECT_OT_run_post_import_pipeline,
